...
var Config = {
    base_url: "https://raw.githubusercontent.com/Exonode/Pokemon-Scripts/master",
    dataDir: "scriptdata/",
    bot: "Dratini",
    kickbot: "Blaziken",
    capsbot: "Exploud",
...
cookieBanned: function(src) {
    ...
    if (script.namesToUnban.get(sys.name(src).toLowerCase()) === "true") {
        ...
        normalbot.sendHtmlMessage(src, "You are currently <font color=red><b><i>banned</i></b></font> from the server. If you believe this to be an error, post here: http://pokemon-online.eu/forums/disciplinary-committee.43/");
        sys.kick(src);
        return true;
...
beforeChatMessage: function(src, message, chan) {
    ...
    */
    ...
    if (sys.auth(src) == 0) {
        sys.sendHtmlAll("<span style='color: " + sys.getColor(src) + "'><timestamp/><b>" + sys.name(src) + ": </b></span>" + message.replace("&", "&amp;").replace("<", "&lt;"),  channel);
        sys.stopEvent();
        this.afterChatMessage(src, message, channel);
        return;
    }
    ...
    if (sys.auth(src) == 1) {
	sys.sendHtmlAll("<span style='color: " + sys.getColor(src) + "'><timestamp/><b><i><font color=cyan>{Moderator}</font></i> " + sys.name(src) + ": </b></span>" + message.replace("&", "&amp;").replace("<", "&lt;"),  channel);
        sys.stopEvent();
        this.afterChatMessage(src, message, channel);
        return;
	}
    if (sys.auth(src) == 2) {
	sys.sendHtmlAll("<span style='color: " + sys.getColor(src) + "'><timestamp/><b><i><font color=gold>{Administrator}</font></i> " + sys.name(src) + ": </b></span>" + message.replace("&", "&amp;").replace("<", "&lt;"),  channel);
        sys.stopEvent();
        this.afterChatMessage(src, message, channel);
        return;
	}
    if (sys.auth(src) == 3) {
	sys.sendHtmlAll("<span style='color: " + sys.getColor(src) + "'><timestamp/><b><i><font color=red>{Owner}</font></i> " + sys.name(src) + ": </b></span>" + message.replace("&", "&amp;").replace("<", "&lt;"),  channel);
        sys.stopEvent();
        this.afterChatMessage(src, message, channel);
        return;
	}
