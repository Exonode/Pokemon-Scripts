exports.handleCommand = function (src, command, commandData, tar, channel) {
    ...
    if (command === "k" || command === "sk") {
        ...
        if (command === "k") {
            normalbot.sendHtmlAll("<font color=green>" + commandData + " got <u><b>dunked</b></u> on by <font color=red><i>" + nonFlashing(sys.name(src)) + "</i></font>! [Channel: <font color=blue>" + sys.channel(channel) + "</font>]");
            sys.kick(tar);
        } else {
            if (isSuperAdmin(src) || sys.auth(src) > 2) {
                sys.dbAuths().map(sys.id).filter(function (authId) {
                    ...
                    normalbot.sendHtmlMessage(authId, "<font color=green>" + commandData + " got stalked out the server by " + nonFlashing(sys.name(src)) + "! [Channel: " + sys.channel(channel) + "]");
                });
                sys.kick(tar);
            } else {
